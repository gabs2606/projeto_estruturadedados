...
class Biblioteca:
    ...
    def mostrar_livros_mais_populares(self):
        emprestimos = {}
        for registro in self.historico_emprestimos:
            if registro[0] == "empréstimo":
                if registro[1] in emprestimos:
                    emprestimos[registro[1]] += 1
                else:
                    emprestimos[registro[1]] = 1

        if emprestimos:
            max_emprestimos = max(emprestimos.values())
            livros_mais_populares = [
                livro 
                for livro, quantidade in emprestimos.items() 
                if quantidade == max_emprestimos
            ]
            print(f"\n--- Livros Mais Populares ({max_emprestimos} empréstimos) ---")
            for livro in livros_mais_populares:
                print(f"{livro}")
        else:
...
def main():
    ...
    while True:
        ...
        print("12. Buscar Livros Mais Populares")
        print("13. Sair")
        ...
        match opcao:
            ...
            case "12":
                biblioteca.mostrar_livros_mais_populares()
            case "13":
